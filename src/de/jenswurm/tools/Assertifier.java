...
public class Assertifier {
    ...
    private Set<Class> filterClasses = new HashSet<Class>();
    
    private List<Class> currentLineClasses=new ArrayList<Class>();
    ...
    public Assertifier filter(Class... classes) {
        filterClasses.addAll(Arrays.asList(classes));
        return this;
    }
	...
	private StringBuilder buildEquals(String result, String path) {
		if (filteredClassesOk()) {
			return new StringBuilder("Assert.assertEquals(" + result + ", "
					+ path + ");\r\n");
		} else {
	...
	private StringBuilder buildNull(String path) {
		// filterClassesOk must be called first
		if (filteredClassesOk() && includeNull) {
    ...
    private String buildPathListEntry(String path, int i, Object result) {
        return "((" + result.getClass().getSimpleName() + ")" + path + ".get(" + i + "))";
    }
    ...
    private StringBuilder processResult(Enum result, String path) {
        return buildEquals(result.getClass().getSimpleName() + "." + result, path);
    }
    ...
    private StringBuilder processResult(Method currMethod, Object result, String path, boolean root)
        ...
        //add the current class to the list...result may be null here
		if (result != null) {
			currentLineClasses.add(result.getClass());
		}
        ...
        if (result == null) {
            ...
            ret.append(processResult((Integer) result, buildPathWithCast(currMethod, result, path)));
        } else if (result instanceof Long) {
            ret.append(processResult((Long) result, buildPathWithCast(currMethod, result, path)));
        } else if (result instanceof String) {
            ret.append(processResult((String) result, buildPathPrimitive(currMethod, result, path)));
        } else if (result instanceof Enum) {
            ret.append(processResult((Enum) result, buildPathPrimitive(currMethod, result, path)));
        ...
        //remove the current element again
		if (result != null) {
			currentLineClasses.remove(currentLineClasses.size() - 1);
		}
    ...
    /**
     * Checks if the list of filtered classes is ok
     * @return
     */
    private boolean filteredClassesOk() {
		if (filterClasses.isEmpty()){
			return true;
		}
		for (Class curr : currentLineClasses) {
			if(filterClasses.contains(curr)){
				return true;
			}
		}
		return false;
    ...
    private StringBuilder processResult(Boolean result, String path) {
        return buildEquals("Boolean.valueOf(" + result + ")", "Boolean.valueOf(" + path + ")");
    }

    /**This method adds object to the list of already traversed results, and checks if they are eligible for that list in first instance. E.g. Strings are not, as the same value may be used in different places, but they are not supposed to be considered object identical 
     * @param result
     */
